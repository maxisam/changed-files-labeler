...
import { getLabels, getPathTokens, getRegexPattern, getTokenSets } from '../src/utils';
...
test('getPathTokens with 1 layers pattern and 1 layers path', () => {
    ...
    expect(tokens).toEqual(['base/foo', 'foo', 'foo']);
});

test('getPathTokens with 2 layers pattern and 2 layers path', () => {
    const pattern = getRegexPattern('base', 2);
    const tokens = getPathTokens('base/foo/bar/somefile.ts', pattern);
    expect(tokens).toEqual(['base/foo/bar', 'foo:bar', 'foo', 'bar']);
});
...
test('getTokenSets with 2 paths and 2 laypers, common sub path', () => {
    ...
    expect(result).toEqual([new Set(['base/foo/bar', 'base/foo/baz']), new Set(['foo:bar', 'foo:baz']), new Set(['foo']), new Set(['bar', 'baz'])]);
});
...
test('getTokenSets with 2 paths and 2 laypers, all different paths', () => {
    ...
    expect(result).toEqual([
        new Set(['base/foo1/bar', 'base/foo2/baz']),
        new Set(['foo1:bar', 'foo2:baz']),
        new Set(['foo1', 'foo2']),
        new Set(['bar', 'baz'])
    ]);
});
...
test('getTokenSets with 2 paths and 1 laypers, all different paths', () => {
    ...
    const pattern = getRegexPattern('base', layers);
    const result = getTokenSets(filePaths, pattern, layers, false);
    expect(result).toEqual([new Set(['base/foo1', 'base/foo2']), new Set(['foo1', 'foo2']), new Set(['foo1', 'foo2'])]);
});
